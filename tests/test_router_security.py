...
def test_auth_token_autogenerated(router_default, monkeypatch, capsys):
    ...
    # Auto-generated token is logged exactly once at startup, but REDACTED
    # to a fingerprint (first 4 + "..." + last 4) — the full token is NOT
    # in the log. Operators verify the token is set via the fingerprint.
    fp = f"{router_default.API_TOKEN[:4]}...{router_default.API_TOKEN[-4:]}"
    assert fp in captured.out, f"expected fingerprint {fp!r} in log output"
    assert router_default.API_TOKEN not in captured.out, (
